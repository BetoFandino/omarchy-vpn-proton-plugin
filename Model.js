...
// ---- Proton VPN (official `protonvpn` CLI) ---------------------------------
//
// Proton is driven by its own CLI rather than NetworkManager profiles the
// plugin imports. The CLI itself creates a WireGuard NetworkManager
// connection ("ProtonVPN <server>", type wireguard), which parseVpnList
// ignores (it only keeps type "vpn"), so Proton never shows up twice.
var PROTON_BIN = "/usr/bin/protonvpn"

// Runs `protonvpn <args...>`. Exits 127 (fails closed) when the CLI isn't
// installed so the panel can hide the Proton row instead of showing an error.
var protonScript =
  "P=" + PROTON_BIN + ";" +
  " if [ ! -x \"$P\" ]; then exit 127; fi;" +
  " exec \"$P\" \"$@\""

// Reads the Proton tunnel's IP and rx/tx counters. Emits the same
// "@@VPN=<key>@@" block shape as statsScript so parseVpnStatsBlocks /
// updateVpnStats can be reused, keyed "proton". The interface is whichever
// /sys/class/net/proton* exists (the CLI names it proton0).
var protonStatsScript =
  "NMCLI=/usr/bin/nmcli; dev='';" +
  " for d in /sys/class/net/proton*; do [ -e \"$d\" ] && dev=${d##*/} && break; done;" +
  " ip=''; rx=0; tx=0;" +
  " if [ -n \"$dev\" ]; then" +
  "   ip=$(\"$NMCLI\" -t -e no -g IP4.ADDRESS device show \"$dev\" 2>/dev/null); ip=${ip%%$'\\n'*};" +
  "   [ -r \"/sys/class/net/$dev/statistics/rx_bytes\" ] && IFS= read -r rx < \"/sys/class/net/$dev/statistics/rx_bytes\";" +
  "   [ -r \"/sys/class/net/$dev/statistics/tx_bytes\" ] && IFS= read -r tx < \"/sys/class/net/$dev/statistics/tx_bytes\";" +
  " fi;" +
  " printf '@@VPN=proton@@\\ndev=%s\\nip=%s\\ngw=\\nrx=%s\\ntx=%s\\n' \"$dev\" \"$ip\" \"$rx\" \"$tx\""

// `protonvpn status` prints "Key: value" lines, e.g.
//   Status: Connected
//   Server: US-FREE#113 in Miami, United States
//   Load: 77%
//   Protocol: wireguard
// It may also print housekeeping lines first ("Server list is outdated,
// updating...") which we simply ignore. Anything that isn't an explicit
// "Status: Connected" is treated as disconnected; a not-signed-in hint is
// surfaced separately so the panel can tell the user to run `protonvpn signin`.
function parseProtonStatus(stdout, stderr) {
  var text = String(stdout || "") + "\n" + String(stderr || "")
  var info = { connected: false, server: "", location: "", load: "", protocol: "", needsSignin: false }
  var lines = text.split(/\r?\n/)

  for (var i = 0; i < lines.length; i++) {
    var line = lines[i].trim()
    var m = line.match(/^(Status|Server|Load|Protocol):\s*(.*)$/i)
    if (!m) continue
    var key = m[1].toLowerCase()
    var value = m[2].trim()
    if (key === "status") info.connected = /^connected$/i.test(value)
    else if (key === "server") {
      var at = value.indexOf(" in ")
      info.server = at === -1 ? value : value.substring(0, at)
      info.location = at === -1 ? "" : value.substring(at + 4)
    }
    else if (key === "load") info.load = value
    else if (key === "protocol") info.protocol = value
  }

  if (!info.connected && /sign ?in|log ?in|not (signed|logged)|authenticat/i.test(text)) info.needsSignin = true
  return info
}

function protonFailureMessage(stdout, stderr) {
  var text = String(stderr || "") + " " + String(stdout || "")
  if (/sign ?in|log ?in|not (signed|logged)|authenticat/i.test(text)) return "Not signed in – run: protonvpn signin"
  if (/no server|not found|no matching/i.test(text)) return "No matching Proton server"
  if (/network is unreachable|could not resolve|name or service not known/i.test(text)) return "No network connection"
  if (/timeout/i.test(text)) return "Connection timed out"
  return elideStatus(text || "Proton VPN failed")
}

// Builds the argv for `protonvpn connect`. The optional country comes from a
// user setting and is passed as its own argv element (never through a
// shell), but is still restricted to letters/spaces/dashes so a stray
// value can't turn into an unexpected CLI option.
function protonConnectArgs(country) {
  var args = ["connect"]
  var c = String(country || "").trim()
  if (c !== "" && /^[A-Za-z][A-Za-z .'-]*$/.test(c)) args.push("--country", c)
  return args
}
...
if (typeof module !== "undefined") {
  module.exports = {
    ...
    protonScript: protonScript,
    protonStatsScript: protonStatsScript,
    parseProtonStatus: parseProtonStatus,
    protonFailureMessage: protonFailureMessage,
    protonConnectArgs: protonConnectArgs
  }
}
